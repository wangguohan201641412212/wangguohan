...
class CardPredictor:
    ...
    def train_svm(self):
        ...
        if os.path.exists("svm.dat"):
            ...
            chars_train = list(map(recognition.deskew, chars_train))
            chars_train = recognition.preprocess_hog(chars_train)
        ...
        if os.path.exists("svmchinese.dat"):
            ...
            for root, dirs, files in os.walk("train\\charsChinese"):
                ...
                index = recognition.provinces.index(pinyin) + PROVINCE_START + 1  # 1是拼音对应的汉字
                for filename in files:
                    filepath = os.path.join(root, filename)
                    digit_img = cv2.imread(filepath)
                    digit_img = cv2.cvtColor(digit_img, cv2.COLOR_BGR2GRAY)
    ...
    def img_first_pre(self, car_pic_file):
        ...
        if type(car_pic_file) == type(""):
            img = commonMath.img_read(car_pic_file)
        else:
            img = car_pic_file
    ...
    def img_color_contours(self, img_contours, oldimg):
        ...
        card_contours = commonMath.img_findContours(img_contours)
        card_imgs = commonMath.img_Transform(card_contours, oldimg, pic_width, pic_hight)
        colors, car_imgs = commonMath.img_color(card_imgs)
        predict_result = []
        roi = None
        card_color = None
        ...
        for i, color in enumerate(colors):
            if color in ("blue", "yello", "green"):
                ...
                wave_peaks = commonMath.find_waves(x_threshold, x_histogram)
                if len(wave_peaks) == 0:
                    print("peak less 0:")
                    continue
                # 认为水平方向，最大的波峰为车牌区域
                ...
                wave_peaks = commonMath.find_waves(y_threshold, y_histogram)
                if len(wave_peaks) <= 6:
                ...
                part_cards = commonMath.seperate_card(gray_img, wave_peaks)
                for i, part_card in enumerate(part_cards):
                    ...
                    part_card = recognition.preprocess_hog([part_card])
                    if i == 0:
                        resp = self.modelchinese.predict(part_card)
                        charactor = recognition.provinces[int(resp[0]) - PROVINCE_START]
                    else:
    ...
    def img_only_color(self, filename, oldimg, img_contours):
        ...
        card_contours = commonMath.img_findContours(img_edge2)
        card_imgs = commonMath.img_Transform(card_contours, oldimg, pic_width, pic_hight)
        colors, car_imgs = commonMath.img_color(card_imgs)
    ...
    def img_mser(self, filename):
        if type(filename) == type(""):
            img = commonMath.img_read(filename)
        else:
            img = filename
        oldimg = img
        ...
        debug.img_show(img)
        colors, car_imgs = commonMath.img_color(colors_img)
        for i, color in enumerate(colors):
            if color != "no":
                print(color)
